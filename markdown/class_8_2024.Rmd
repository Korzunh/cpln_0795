...
```{r}
census_api_key("7d94a77ffab0e66527d76b9f4a8c557f41b47597", overwrite = TRUE, install = TRUE)
```
...
acs2020_incidents <- acs2020_incidents %>%
  mutate(area = st_area(.))
glimpse(acs2020_incidents)
class(acs2020_incidents$area)
```
...
```{r}
philly_areawater <- area_water("PA", "Philadelphia", year = 2020)

```
```{r}
st_crs(philly_areawater)
philly_areawater <- philly_areawater %>%
  st_transform(crs = 2272)
```
...
They are hosted at this URL - council_dist <- st_read("https://raw.githubusercontent.com/mafichman/cpln_0795/main/data/councildist.geojson")
...
ggplot()+
  geom_sf(data = acs2020_incidents,
          aes(fill = n))+
  geom_sf(data = philly_areawater)+
  geom_sf(data = council_dist, fill = "transparent", color = "white")+
    theme_minimal()+
  labs(
    title = "311 Calls by Council District"
  )+
